...
//         UserModel.getMulti({ _id: { $in: uids } }, ['acm', 'realname_flag', 'realname_name', '_id', 'uname', 'mail', 'avatar', 'school', 'studentId', 'certification_flag', 'certification_adder']).toArray(),
...
//实名设置
class CertificationSetHandler extends Handler {
    async get() {
        this.response.template = 'certification_set.html'; // 返回此页面
    }
    @param('uidOrName', Types.UidOrName)
    @param('flag', Types.number)
    async post(domainId: string, uidOrName: string, flag: number) {
        // 检查输入
        flag = parseInt(flag);
        ...
        // 构建徽章代码并更新
        await UserModel.setById(udoc._id, { certification_flag: flag, certification_adder: this.user._id });
        // 将用户重定向到创建完成的url
        this.response.redirect = "/certification/show";
    }
}

//展示实名用户
class CertificationShowHandler extends Handler {
    @query('page', Types.PositiveInt, true)
    async get(domainId: string, page = 1) {
        const [dudocs, upcount, ucount] = await this.ctx.db.paginate(
            UserModel.getMulti({ certification_flag: { $exists: true } }).sort({ certification_flag: -1, _id: 1 }),
            page,
            50,
        );
        const udict = await UserModel.getList("system", dudocs.map((x) => x._id));
        const udocs = dudocs.map((x) => udict[x._id]);
        this.response.template = 'certification_show.html'; // 返回此页面
...
export async function apply(ctx: Context) {
    ctx.inject(['setting'], (c) => {
        c.setting.AccountSetting(
            SettingModel.Setting('setting_info', 'certification_flag', 0, 'number', 'certification_flag', null, 3),
            SettingModel.Setting('setting_info', 'certification_adder', 2, 'number', 'certification_adder', null, 3),
        );
        ctx.Route('certification_set', '/certification/set', CertificationSetHandler, PRIV.PRIV_MANAGE_USER_INFO);
        ctx.Route('certification_show', '/certification/show', CertificationShowHandler, PRIV.PRIV_MANAGE_USER_INFO);
        global.Hydro.ui.inject('DomainManage', 'certification_set', {
            family: 'Certification Settings',
            icon: 'settings',
        });
        global.Hydro.ui.inject('DomainManage', 'certification_show', {
            family: 'Certification Settings',
            icon: 'settings',
        });
    });
